...
const onSubmit = async (data:Register)=>{
  ...
  try{ 
      const res:any = await axios.post('/api/users/signup',data);
      reset()
      setError("")
  } catch(err:any){
  ...
  return (
    <div className="min-h-screen flex justify-center items-center">
      <div className="border border-[#27272A] p-5 rounded-lg w-[600px]">
        ...
        <form className="flex flex-col gap-4 mt-10" onSubmit={handleSubmit(onSubmit)}>
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              {...register("userName", { required: 'Username is required',min:5, maxLength: 100 })}
            />
        ...
        <div className="mt-10">
            <p className="text-white/60">Already have an account?<Link href="/log-in" className="text-blue-300">&nbsp; Login</Link></p>
        </div>
      </div>
    </div>
  );
